...
export const Route = createFileRoute("/_app/psur")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Upload a PSUR/PBRER, extract its structure and review completeness, consistency and numerical findings." },
...
function PsurPage() {
  const docs = usePvQuery(["psur", "documents"], () => psurApi.documents(), () => demoPsurDocuments);
      ...
      <div className="space-y-4 p-6">
          ...
          <ol className="flex flex-wrap items-center gap-2 text-sm">
            {FLOW.map((s, i) => (
              <li key={s} className="flex items-center gap-2">
                ...
                {i < FLOW.length - 1 ? <ArrowRight className="size-3.5 text-muted-foreground" /> : null}
              </li>
            ))}
          </ol>
        </Section>

        <Section title="Upload a periodic report" description="PDF only. Extraction and analysis run server-side.">
          <label className="flex cursor-pointer flex-col items-center gap-2 rounded-md border border-dashed border-border px-6 py-8 text-center hover:bg-muted/50">
            ...
            <span className="text-sm font-medium">{uploading ? "Uploading…" : "Choose a PDF"}</span>
            <input
              type="file"
              ...
              onChange={async (e) => {
                ...
                try {
                  ...
                  toast.error(
                    isNotConfigured(err) ? "Backend not connected — the document was not uploaded." : "Upload failed.",
                  );
                } finally {
                  setUploading(false);
                }
        ...
        <Section title="Documents">
          <QueryBoundary query={docs}>
            {(items) =>
              items.length === 0 ? (
                <EmptyState title="No documents" description="Upload a PSUR/PBRER to begin a review." />
              ) : (
                <ul className="divide-y divide-border">
                  {items.map((d) => (
                    <li key={d.id} className="flex flex-wrap items-center gap-3 py-3">
                      ...
                      <StatusPill tone={d.stage === "FAILED" ? "critical" : d.stage === "REVIEWED" ? "success" : "info"}>
                        {d.stage.toLowerCase()}
                      </StatusPill>
                      <span className="mono-num text-xs text-muted-foreground">{d.pages} pages</span>
                      <Button size="sm" variant="outline" className="ml-auto" onClick={() => setSelected(d.id)}>
                        Open review
                      </Button>
                    </li>
                  ))}
          ...
          <>
            <Section title="Document metadata">
              <div className="grid gap-4 sm:grid-cols-4">
                ...
                <Field label="Uploaded by" value={`${activeDoc.uploadedBy} · ${activeDoc.uploadedAt.slice(0, 10)}`} />
              </div>
            </Section>
                      ...
                      {items.map((f) => (
                        <li key={f.id} className="rounded-md border border-border p-3">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <StatusPill tone={f.severity === "HIGH" ? "critical" : f.severity === "MEDIUM" ? "warning" : "neutral"}>
                              {f.severity.toLowerCase()} severity
                            </StatusPill>
                            <span className="text-sm font-medium">{f.section}</span>
                            {f.assistGenerated ? <AssistLabel>AI-generated review assistance</AssistLabel> : null}
                          </div>
                          <p className="mt-2 text-sm">{f.description}</p>
                          <p className="mt-1 border-l-2 border-border pl-2 text-xs text-muted-foreground">{f.evidence}</p>
                          <div className="mt-3 flex flex-wrap gap-2">
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  await psurApi.recordAssessment(activeDoc.id, f.id, "ACCEPTED", "Confirmed by reviewer");
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  await psurApi.recordAssessment(activeDoc.id, f.id, "DISMISSED", "Not applicable");
                                  toast.success("Assessment recorded.");
                                } catch (err) {
                                  toast.error(
                                    isNotConfigured(err)
