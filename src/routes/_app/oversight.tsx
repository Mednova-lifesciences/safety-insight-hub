...
import { demoAudit, demoCases, demoPsurDocuments, demoSignals } from "@/services/demo/dataset";
  ...
  head: () => ({
    meta: [
      { title: "Operational overview — MedNova PV Assist" },
      {
        name: "description",
        content:
          "Administrator view of case volume, serious and overdue cases, coding backlog, signals and audit activity.",
      },
      { property: "og:title", content: "Operational overview — MedNova PV Assist" },
      {
        property: "og:description",
        content: "Pharmacovigilance operations oversight and drill-down.",
      },
    ],
  }),
  component: () => (
    ...
    <>
      ...
      <div className="space-y-4 p-6">
        <QueryBoundary query={casesQuery} loadingLabel="Loading operational data">
          {(all) => {
            ...
            const seriousSplit = [
              ...
              {
                name: "Non-serious",
                value: all.filter((c) => c.seriousness === "NON_SERIOUS").length,
              },
              {
                name: "Unassessed",
                value: all.filter((c) => c.seriousness === "UNASSESSED").length,
              },
            ];
            ...
            return (
              <>
                <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-6">
                  ...
                  <Metric
                    label="Serious"
                    value={all.filter((c) => c.seriousness === "SERIOUS").length}
                    tone="text-critical"
                    to="/cases"
                  />
                  <Metric
                  ...
                  <Metric
                    ...
                    value={all.filter((c) => c.workflowStep === "REVIEW").length}
                    to="/cases"
                  />
                  <Metric
                    label="Coding backlog"
                    value={all.filter((c) => c.workflowStep === "CODING").length}
                    tone="text-warning"
                    to="/cases"
                  />
                </div>
                ...
                <Section
                  title="Seriousness flags"
                  description="Assistive mismatches awaiting human review."
                >
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          <Section
            title="Signals"
            description="Drill into the signal workspace for evidence and decisions."
          >
                ...
                <ul className="divide-y divide-border">
                  {items.map((s) => (
                    <li key={s.id} className="flex flex-wrap items-center gap-2 py-2.5">
                      ...
                      <Link to="/signals" className="text-sm text-primary hover:underline">
                        Open
                      </Link>
                    </li>
                  ))}
                ...
                <ul className="divide-y divide-border">
                  {items.map((d) => (
                    <li key={d.id} className="flex flex-wrap items-center gap-2 py-2.5">
                      ...
                      <StatusPill tone={d.stage === "REVIEWED" ? "success" : "info"}>
                        {d.stage.toLowerCase()}
                      </StatusPill>
                      <Link to="/psur" className="text-sm text-primary hover:underline">
                        Open
                      </Link>
                    </li>
                  ))}
