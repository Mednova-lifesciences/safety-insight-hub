...
| D8 | C.1.7 — does the case fulfil local criteria for an expedited report? | **Rule agreed and in force** | Seriousness decides it: a case meeting any ICH/WHO seriousness criterion (death, life-threatening, hospitalisation, disability, congenital anomaly, other medically important) or recorded as serious by the file is recommended YES, plus the AEFI triggers WHO and Nigeria add (cluster, immunization error, community concern). Every framework checked expedites a serious case: ICH E2D, EU GVP VI, US FDA, NAFDAC GVP 2021 §5.72 (serious unexpected 72 hours, serious expected 15 days) and WHO/Nigeria AEFI surveillance (serious AEFI notified within 24 hours). Unexpectedness is deliberately not judged — it needs the product's labelling, which a line list does not carry. The rule is data, not code (`services/e2b-r3/c17-rule.ts`): any qualified assessor edits it under Settings → C.1.7, each change a new version signed with their password (`pv_e2b_c17_rules`). It RECOMMENDS only; a qualified assessor still finalizes every case, and "accept the recommendations" records one audited decision per case. Where a line list never states seriousness, the model reads that case and suggests an answer — stored apart from the assessment, never able to finalize |
